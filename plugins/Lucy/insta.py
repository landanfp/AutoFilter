...
async def link_handler(Mbot, message):
    ...
    try:
        ...
        if url.endswith("="):
           dump_file=await message.reply_video(url[:-1],caption="✅ @IR_InstagramDLBot")
        else:
            dump_file=await message.reply_video(url,caption="✅ @IR_InstagramDLBot")
        ...
        try:
            if "/reel/" in url:
               ...
               try:
                   ...
                   dump_file=await message.reply_video(downfile,caption="**✅ @IR_InstagramDLBot**") 
            elif "/p/" in url:
                  meta_tag = requests.post("https://saveig.app/api/ajaxSearch", data={"q": link, "t": "media", "lang": "en"}, headers=headers)
                  if meta_tag.ok:
                     res=meta_tag.json()
              ...
              #    await message.reply(meta)
                  for i in range(len(meta) - 1):
                     ...
                     try:
                        dump_file=await message.reply_video(com.text,caption="**✅ @IR_InstagramDLBot**")
                        await com.delete()
                     except:
                         pass 
            elif "stories" in url:
          ...
          #     await message.reply(tracemsg)
            ...
            if 'downfile' in locals():
                os.remove(downfile)
            await message.reply("<a href='https://t.me/ir_botz'>سایر ربات ها</a>")
